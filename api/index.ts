import express from 'express';

const app = express();
app.use(express.json());

// CORS & Security headers
app.use((req, res, next) => {
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Access-Control-Allow-Methods', 'GET, POST, OPTIONS');
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Authorization');
  if (req.method === 'OPTIONS') return res.status(200).end();
  next();
});

const LASTFM_API_KEY = process.env.LASTFM_API_KEY || 'f21088bf9097b49ad4e7f487abab981e';
const LASTFM_BASE = 'https://ws.audioscrobbler.com/2.0/';

const listenMemoryStore: Map<string, any[]> = new Map();

function makeSongId(artist: string, title: string): string {
  const clean = `${(artist || '').trim().toLowerCase()}:::${(title || '').trim().toLowerCase()}`;
  return `lastfm_${Buffer.from(clean).toString('base64url')}`;
}

function decodeSongId(id: string): { artist: string; title: string } | null {
  if (!id || !id.startsWith('lastfm_')) return null;
  try {
    const raw = Buffer.from(id.slice(7), 'base64url').toString('utf8');
    const [artist, title] = raw.split(':::');
    if (artist && title) return { artist, title };
  } catch {}
  return null;
}

function extractLastFmImage(images: any[] | undefined): string | null {
  if (Array.isArray(images) && images.length > 0) {
    const extralarge = images.find((i: any) => i.size === 'extralarge' || i.size === 'mega');
    const large = images.find((i: any) => i.size === 'large');
    const medium = images.find((i: any) => i.size === 'medium');
    const url = extralarge?.['#text'] || large?.['#text'] || medium?.['#text'] || images[images.length - 1]?.['#text'] || '';
    if (
      url &&
      typeof url === 'string' &&
      url.trim().length > 0 &&
      !url.includes('2a96cbd8b46e442fc41c2b86b821562f') &&
      url.startsWith('http')
    ) {
      return url.trim();
    }
  }
  return null;
}

const itunesCoverCache = new Map<string, string>();
async function getItunesArtwork(artist: string, title: string): Promise<string> {
  const cleanTitle = (title || '').replace(/\(.*?\)/g, '').replace(/\[.*?\]/g, '').trim();
  const cleanArtist = (artist || '').split(',')[0].split('&')[0].trim();
  const cacheKey = `${cleanArtist.toLowerCase()}:::${cleanTitle.toLowerCase()}`;
  if (itunesCoverCache.has(cacheKey)) return itunesCoverCache.get(cacheKey)!;

  try {
    const url = `https://itunes.apple.com/search?term=${encodeURIComponent(`${cleanArtist} ${cleanTitle}`)}&entity=song&limit=1`;
    const res = await fetch(url, { signal: AbortSignal.timeout(3000) });
    if (res.ok) {
      const data = await res.json();
      const first = data.results?.[0];
      if (first?.artworkUrl100) {
        const upgraded = first.artworkUrl100.replace('100x100bb', '600x600bb');
        itunesCoverCache.set(cacheKey, upgraded);
        return upgraded;
      }
    }
  } catch {}
  return '';
}

async function callLastFm(method: string, params: Record<string, string>): Promise<any> {
  const queryParams = new URLSearchParams({
    method,
    api_key: LASTFM_API_KEY,
    format: 'json',
    ...params,
  });

  const response = await fetch(`${LASTFM_BASE}?${queryParams.toString()}`, {
    headers: { 'User-Agent': 'WaveMusicApp/2.0' },
    signal: AbortSignal.timeout(5000),
  });

  if (!response.ok) throw new Error(`Last.fm API returned HTTP ${response.status}`);
  return await response.json();
}

async function formatLastFmTrack(track: any): Promise<any> {
  if (!track || (!track.name && !track.title)) return null;
  const title = track.name || track.title || 'Unknown Title';
  const artist = typeof track.artist === 'string' ? track.artist : track.artist?.name || 'Unknown Artist';
  const duration = track.duration ? Number(track.duration) : 210;

  let coverUrl = extractLastFmImage(track.image) || '';
  if (!coverUrl) coverUrl = await getItunesArtwork(artist, title);

  const id = makeSongId(artist, title);
  return {
    id,
    title,
    artist,
    album: track.album?.title || track.album?.['#text'] || 'Single',
    duration,
    coverUrl,
    streamUrl: `youtube:${encodeURIComponent(`${artist} ${title} audio`)}`,
    quality: 'Full Track',
    source: 'lastfm',
    listeners: track.listeners ? String(track.listeners) : undefined,
    playcount: track.playcount ? String(track.playcount) : undefined,
  };
}

// Health check
app.all(['/api/health', '/health', '/healthz'], (req, res) => {
  res.json({ status: 'ok', engine: 'lastfm + youtube', hasKey: Boolean(LASTFM_API_KEY) });
});

// Search
app.all(['/api/music/search', '/music/search', '/api/search/songs', '/search/songs'], async (req, res) => {
  const query = (req.query.query || req.query.q || '').toString().trim();
  if (!query) return res.json({ success: true, data: { total: 0, results: [] } });

  try {
    const data = await callLastFm('track.search', { track: query, limit: '30' });
    const rawTracks = data?.results?.trackmatches?.track || [];
    const tracksArray = Array.isArray(rawTracks) ? rawTracks : [rawTracks];
    const results = (await Promise.all(tracksArray.map(formatLastFmTrack))).filter(Boolean);
    if (results.length > 0) {
      return res.json({ success: true, data: { total: results.length, results } });
    }
  } catch {}

  // Fallback: iTunes search
  try {
    const itunesUrl = `https://itunes.apple.com/search?term=${encodeURIComponent(query)}&entity=song&limit=30`;
    const itunesRes = await fetch(itunesUrl, { signal: AbortSignal.timeout(4000) });
    if (itunesRes.ok) {
      const itunesData = await itunesRes.json();
      const results = (itunesData.results || []).map((t: any) => ({
        id: makeSongId(t.artistName, t.trackName),
        title: t.trackName,
        artist: t.artistName,
        album: t.collectionName || 'Single',
        duration: Math.round((t.trackTimeMillis || 210000) / 1000),
        coverUrl: t.artworkUrl100?.replace('100x100bb', '600x600bb') || '',
        streamUrl: `youtube:${encodeURIComponent(`${t.artistName} ${t.trackName} audio`)}`,
        quality: 'Full Track',
        source: 'lastfm',
      }));
      return res.json({ success: true, data: { total: results.length, results } });
    }
  } catch {}

  res.json({ success: true, data: { total: 0, results: [] } });
});

// Curated & Charts
app.all(['/api/curated', '/curated', '/api/charts/tracks', '/charts/tracks'], async (req, res) => {
  try {
    const data = await callLastFm('chart.gettoptracks', { limit: '30' });
    const rawTracks = data?.tracks?.track || [];
    const tracksArray = Array.isArray(rawTracks) ? rawTracks : [rawTracks];
    const results = (await Promise.all(tracksArray.map(formatLastFmTrack))).filter(Boolean);
    if (results.length > 0) return res.json({ success: true, results });
  } catch {}

  // iTunes Top Songs RSS fallback
  try {
    const itunesRes = await fetch('https://itunes.apple.com/us/rss/topsongs/limit=30/json', { signal: AbortSignal.timeout(4000) });
    if (itunesRes.ok) {
      const itunesData = await itunesRes.json();
      const entries = itunesData.feed?.entry || [];
      const results = entries.map((e: any) => {
        const title = e['im:name']?.label || 'Unknown Track';
        const artist = e['im:artist']?.label || 'Unknown Artist';
        const rawCover = e['im:image']?.[e['im:image']?.length - 1]?.label || '';
        return {
          id: makeSongId(artist, title),
          title,
          artist,
          album: e['im:collection']?.['im:name']?.label || 'Single',
          duration: 210,
          coverUrl: rawCover.replace(/\/\d+x\d+bb/, '/600x600bb'),
          streamUrl: `youtube:${encodeURIComponent(`${artist} ${title} audio`)}`,
          quality: 'Full Track',
          source: 'lastfm',
        };
      });
      return res.json({ success: true, results });
    }
  } catch {}

  res.json({ success: true, results: [] });
});

// YouTube Video ID resolver (Fast regex scraper)
app.all(['/api/music/youtube-id', '/music/youtube-id', '/api/youtube-id', '/youtube-id'], async (req, res) => {
  const query = (req.query.query || req.query.q || '').toString().trim();
  if (!query) return res.status(400).json({ error: 'Query required' });

  try {
    const cleanQ = query.replace(/[^\w\s]/gi, ' ').trim();
    const searchUrl = `https://www.youtube.com/results?search_query=${encodeURIComponent(`${cleanQ} audio`)}`;
    const ytRes = await fetch(searchUrl, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
        'Accept-Language': 'en-US,en;q=0.9',
      },
      signal: AbortSignal.timeout(4500),
    });

    if (ytRes.ok) {
      const html = await ytRes.text();
      const match = html.match(/\/watch\?v=([a-zA-Z0-9_-]{11})/);
      if (match && match[1]) {
        const youtubeId = match[1];
        return res.json({
          success: true,
          youtubeId,
          title: query,
          duration: 210,
          coverUrl: `https://img.youtube.com/vi/${youtubeId}/hqdefault.jpg`,
        });
      }
    }
  } catch (e) {}

  res.status(404).json({ error: 'No video found' });
});

// Regional charts
app.all(['/api/charts/regional', '/charts/regional'], async (req, res) => {
  const country = (req.query.country || 'united states').toString();
  try {
    const data = await callLastFm('geo.gettoptracks', { country, limit: '20' });
    const rawTracks = data?.tracks?.track || [];
    const tracksArray = Array.isArray(rawTracks) ? rawTracks : [rawTracks];
    const results = (await Promise.all(tracksArray.map(formatLastFmTrack))).filter(Boolean);
    return res.json({ success: true, results });
  } catch {}
  res.json({ success: true, results: [] });
});

// Artists
app.all(['/api/charts/artists', '/charts/artists'], async (req, res) => {
  try {
    const data = await callLastFm('chart.gettopartists', { limit: '15' });
    const rawArtists = data?.artists?.artist || [];
    const artists = (Array.isArray(rawArtists) ? rawArtists : [rawArtists]).map((a: any) => ({
      name: a.name,
      listeners: a.listeners,
      playcount: a.playcount,
      imageUrl: extractLastFmImage(a.image) || '',
      url: a.url,
    }));
    return res.json({ success: true, artists });
  } catch {}
  res.json({ success: true, artists: [] });
});

// Lyrics
app.all(['/api/lyrics', '/lyrics'], async (req, res) => {
  const artist = (req.query.artist || '').toString().trim();
  const title = (req.query.title || '').toString().trim();
  if (!title) return res.json({ success: false, lyrics: null });

  try {
    const cleanTitle = title.replace(/\(.*?\)/g, '').replace(/\[.*?\]/g, '').trim();
    const cleanArtist = artist.split(',')[0].split('&')[0].trim();
    const lrcUrl = `https://lrclib.net/api/get?artist_name=${encodeURIComponent(cleanArtist)}&track_name=${encodeURIComponent(cleanTitle)}`;
    const lrcRes = await fetch(lrcUrl, { headers: { 'User-Agent': 'WaveMusicApp/2.0' }, signal: AbortSignal.timeout(4000) });
    if (lrcRes.ok) {
      const data = await lrcRes.json();
      return res.json({
        success: true,
        synced: Boolean(data.syncedLyrics),
        syncedLyrics: data.syncedLyrics || '',
        plainLyrics: data.plainLyrics || '',
      });
    }
  } catch {}
  res.json({ success: false, synced: false, plainLyrics: 'No synchronized lyrics found.' });
});

// Suggestions
app.all(['/api/songs/:id/suggestions', '/songs/:id/suggestions', '/api/suggestions', '/suggestions'], async (req, res) => {
  const songId = (req.params.id || req.query.id || '').toString();
  let artist = (req.query.artist || '').toString();
  let title = (req.query.title || req.query.track || '').toString();

  if ((!artist || !title) && songId) {
    const decoded = decodeSongId(songId);
    if (decoded) {
      artist = decoded.artist;
      title = decoded.title;
    }
  }

  try {
    if (artist && title) {
      const data = await callLastFm('track.getsimilar', { artist, track: title, limit: '15' });
      const raw = data?.similartracks?.track || [];
      const tracksArray = Array.isArray(raw) ? raw : [raw];
      const results = (await Promise.all(tracksArray.map(formatLastFmTrack))).filter(Boolean);
      if (results.length > 0) return res.json({ success: true, data: results });
    }
  } catch {}
  res.json({ success: true, data: [] });
});

// Listen Memory
app.all(['/api/listen-memory', '/listen-memory'], (req, res: any) => {
  const userId = (req.query.userId || req.body?.userId || 'guest').toString();
  if (req.method === 'POST') {
    const { songId, title, artist, coverUrl, duration } = req.body || {};
    if (songId) {
      let list = listenMemoryStore.get(userId) || [];
      list = list.filter((i: any) => i.songId !== songId);
      list.unshift({ userId, songId, title: title || 'Unknown', artist: artist || 'Unknown', coverUrl: coverUrl || '', playedAt: Date.now(), duration });
      if (list.length > 100) list = list.slice(0, 100);
      listenMemoryStore.set(userId, list);
    }
    return res.json({ success: true });
  }
  const history = listenMemoryStore.get(userId) || [];
  res.json({ success: true, data: history });
});

export default app;
